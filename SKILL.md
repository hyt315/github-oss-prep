...
description: Use when preparing, publishing, launching, or improving any project for open-source adoption on GitHub. Features 6 archetype-tailored README engines (AI Skill, CLI tool, Multimedia, SDK, Fullstack App, Docs), modern 2026 CLI runners (uvx, uv tool, bunx, pnpm dlx, cargo binstall, brew, winget), 5-layer deep security scanning, YAML Issue Forms, CI/Dependabot templates, and approval-gated publishing. Triggers include GitHub 开源准备, 准备发布到 GitHub, 美化项目准备开源, 开源化, 开源推广, oss prep, publish to GitHub, launch an open-source project, and prepare for open source.
metadata:
  author: hyt315
---
...
- **按六大品类精准针对性适配**：对 AI Skill、CLI 系统工具、前端多媒体、类库 SDK、全栈应用、文档知识库提供匹配度 100% 的开箱即用门面。
...
- **2026 现代终端分发与安装矩阵**：支持 `uvx`、`uv tool`、`pipx`、`npx`、`pnpm dlx`、`bunx`、`cargo binstall`、`winget`、`brew`、`gh skill` 与 Release Checksums。
...
```
Step 0: 定位与品类精准识别 (AI Skill / CLI 工具 / 多媒体 / SDK / 全栈应用 / 文档)
    ↓
Step 1: 五重深度安全扫描 (凭据 + 私有路径 + 会话标记 + Git 污染 + 构建产物)
    ↓
Step 2: 社区文件与 CI 补全 (YAML Issue Forms + PR 模板 + SECURITY + CI/Dependabot)
    ↓
Step 3: 专属针对性 README 门面生成 (六大品类完整模板引擎 + 动态徽章)
    ↓
Step 4: 现代分发渠道精准适配 (uvx / bunx / cargo binstall / brew / gh skill / Checksums)
    ↓
...
### Step 0: 定位与品类精准识别
- 确定项目属于哪一类，并匹配对应的分发渠道：
  - **AI Agent 技能**：走 Agent 一句话自装 / `gh skill install` / skills 目录克隆；
  - **Python 终端工具**：走 `uvx` (免装) / `uv tool install` / `pipx`；
  - **Node 终端工具**：走 `npx` / `pnpm dlx` / `bunx`；
  - **独立编译工具**：走 `brew` / `winget` / `scoop` / `cargo binstall` / Release 预编译包。
...
- 补齐 `CODE_OF_CONDUCT.md`、`CONTRIBUTING.md`、`SECURITY.md`、Node/Python 矩阵 CI 工作流、`dependabot.yml`、`.editorconfig` 与 `.gitattributes`。
...
- 依据 Step 0 确定的项目品类，从 6 大专属完整模板中生成高转化率中英文门面：
  ...
  - **CLI / 系统工具**：痛点解构 + ASCII 架构图 + 6 大实战速查表 + 现代终端运行器表 (`uvx`/`bunx`/`brew`) + Zero-Harm 只读安全铁律。
  - **前端 / 多媒体**：视觉效果 Gallery + 画布比例矩阵 + 宽泛依赖区间。
  - **类库 / SDK**：5 行极速 Hello World + API 参数表 + 语言包管理器安装表 + SemVer 兼容承诺。
  - **全栈应用**：Live Demo + 1 键部署 + Docker 命令 + `.env.example` 环境变量表。
- 详见 [六大品类 README 模板库](references/readme-template.md)。

### Step 4: 现代分发渠道与包管理实操
- 针对性适配发版渠道：`uv publish`、`npm publish`、`cargo publish`、Docker/ghcr.io、Homebrew、`gh skill` 与 Release SHA-256 校验和。
...
- 需要为项目选择并生成专属风格 README 时，先读 [六大品类 README 模板库](references/readme-template.md)：涵盖 6 大项目形态完整开箱即用 Markdown 骨架与 2026 现代终端运行器表（预计阅读时间：4 分钟）。
...
- 需要配置 uv/npm/pip/cargo/docker/brew 多平台发版与 Release 校验和时，先读 [全渠道分发与发版实操指南](references/release-and-distribution.md)：涵盖全生态发版命令、配置文件与国内镜像源（预计阅读时间：4 分钟）。
